...
static int FindByName(const struct Contact* ps, const char* pname)//该函数只能在本文件使用，用于支持其他功能函数
{
	int i = 0;
	for (i = 0; i < ps->size; i++)
	{
...
{
	return ((struct PeoInfo*)e1)->age - ((struct PeoInfo*)e2)->age;//两种找结构体成员的方法都可以
														   //但要注意.和->的优先级高于*
														   //因此将其括起来
}
